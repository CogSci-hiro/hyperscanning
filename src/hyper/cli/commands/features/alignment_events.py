"""CLI command for exporting alignment tiers as onset event tables."""

from __future__ import annotations

import argparse
from pathlib import Path
from typing import Any

from hyper.features.pipelines.acoustics import run_alignment_event_pipeline


def add_subparser(subparsers: Any) -> None:
    """Register the `alignment-events` subcommand."""
    parser = subparsers.add_parser(
        "alignment-events",
        help="Export interval onsets from an alignment CSV tier as an event table.",
    )
    parser.add_argument("--config", type=Path, required=True, help="Path to config YAML.")
    parser.add_argument("--alignment", type=Path, required=True, help="Alignment CSV file.")
    parser.add_argument("--tier", type=str, required=True, help="Tier name to export.")
    parser.add_argument("--feature-name", type=str, required=True, help="Feature name stored in metadata.")
    parser.add_argument("--out-tsv", type=Path, required=True, help="Output TSV event table path.")
    parser.add_argument("--out-sidecar", type=Path, required=True, help="Output JSON sidecar path.")
    parser.add_argument("--source-subject", type=str, default=None, help="Optional source subject stored in metadata.")
    parser.add_argument("--source-role", type=str, default=None, help="Optional source role stored in metadata.")
    parser.add_argument(
        "--exclude-label",
        action="append",
        default=None,
        help="Interval label to exclude. Can be provided multiple times.",
    )


def run(args: argparse.Namespace, cfg) -> None:
    """Execute the `alignment-events` command."""
    del cfg
    run_alignment_event_pipeline(
        alignment_path=args.alignment,
        tier_name=str(args.tier),
        output_tsv_path=args.out_tsv,
        output_sidecar_path=args.out_sidecar,
        feature_name=str(args.feature_name),
        exclude_labels=tuple(args.exclude_label or ()),
        source_subject=args.source_subject,
        source_role=args.source_role,
    )
