...
from duet.viz.pos_qc import RUN_GROUPING_CHOICES, load_pos_qc_dataset, write_pos_qc_outputs
...
def _resolve_input_paths(args: argparse.Namespace) -> list[Path]:
    ...
    if len(deduplicated_paths) == 0:
        raise ValueError("Provide at least one --input or --glob for `duet pos-qc`.")
