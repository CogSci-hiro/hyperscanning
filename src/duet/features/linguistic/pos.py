...
def default_stanza_resources_dir() -> Path:
    ...
    if xdg_cache_home:
        return Path(xdg_cache_home).expanduser() / "duet" / "stanza"
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Caches" / "duet" / "stanza"
    return Path.home() / ".cache" / "duet" / "stanza"
