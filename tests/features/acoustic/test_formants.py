...
from features.acoustic.formants import (
...
def test_vowel_median_computation_uses_mocked_tracks(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "features.acoustic.formants.compute_formant_tracks",
        lambda **kwargs: mocked_tracks,
    )

    from features.acoustic.formants import VowelInterval
    ...
    assert result.metadata.voxatlas_function.endswith("compute_formant_tracks")
...
def test_failed_formant_estimation_is_marked_in_output(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "features.acoustic.formants.compute_formant_tracks",
        lambda **kwargs: pd.DataFrame(columns=["phoneme_id", "F1", "F2"]),
    )
